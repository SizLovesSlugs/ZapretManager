...
func (w *webview) CreateWithOptions(opts WindowOptions) bool {
	...
	// Dark host background (#0a0b0e) so WM_ERASEBKGND is never white.
	bgBrush, _, _ := w32.Gdi32CreateSolidBrush.Call(0x000E0B0A)
	wc := w32.WndClassExW{
	...
	// Show immediately with dark class brush (HbrBackground); avoids white flash
	// without keeping the HWND hidden (hidden create breaks WebView bounds).
...
// Show ensures the native window and WebView controller are visible.
func (w *webview) Show() {
	_, _, _ = w32.User32ShowWindow.Call(w.hwnd, w32.SWShow)
	_, _, _ = w32.User32UpdateWindow.Call(w.hwnd)
	_, _, _ = w32.User32SetFocus.Call(w.hwnd)
